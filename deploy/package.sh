...
echo "3. 重命名 APK"
echo "============================================"
APK_DIR="$PROJECT_DIR/app/build/outputs/apk/lightningPlus/release"
APK_FILE=$(ls "$APK_DIR"/*release*.apk 2>/dev/null | head -1)

if [ -z "$APK_FILE" ]; then
    echo "❌ 未找到 release APK 文件"
    exit 1
fi

# 重命名为 zhaomo_browser.apk
